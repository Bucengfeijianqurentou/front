        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '开始报废数量' : ''">
                        <el-input v-model="searchForm.baofeiNumberStartSearch"
                               placeholder="请输入开始报废数量" clearable></el-input>
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                        =》
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '结束报废数量' : ''">
                        <el-input v-model="searchForm.baofeiNumberEndSearch"
                               placeholder="请输入结束报废数量" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('baofei','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanzichan/upload/baofeiMuBan.xls"
                        >批量导入资产报废数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('baofei','导入导出')"
                                ...
                                :on-success="baofeiUploadSuccess"
                                :on-error="baofeiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('baofei','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入资产报废数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('baofei','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "baofei.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content" style="overflow: visible; width: 100%;">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column min-width="220" :align="contents.tableAlign"
                                     header-align="center"
                                     label="操作">
                        <template slot-scope="scope">
                            <div style="display: flex; justify-content: center; align-items: center; gap: 5px;">
                                <el-button v-if="isAuth('baofei','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                <el-button v-if="isAuth('baofei','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                                <el-button v-if="isAuth('baofei','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            </div>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.baofeiNumberStartSearch != '' && this.searchForm.baofeiNumberStartSearch != undefined) {
                    params['baofeiNumberStartSearch'] = this.searchForm.baofeiNumberStartSearch
                }
                if (this.searchForm.baofeiNumberEndSearch != '' && this.searchForm.baofeiNumberEndSearch != undefined) {
                    params['baofeiNumberEndSearch'] = this.searchForm.baofeiNumberEndSearch
                }
            ...
            // 导入功能上传文件成功后调用导入方法
            baofeiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "baofei/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
