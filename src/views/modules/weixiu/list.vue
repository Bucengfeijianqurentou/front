<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '开始维修数量' : ''">
                        <el-input v-model="searchForm.weixiuNumberStartSearch"
                               placeholder="请输入开始维修数量" clearable></el-input>
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                        =》
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '结束维修数量' : ''">
                        <el-input v-model="searchForm.weixiuNumberEndSearch"
                               placeholder="请输入结束维修数量" clearable></el-input>
                    </el-form-item>
                        
                                         
                    <el-form-item :label="contents.inputTitle == 1 ? '资产编号' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.shangpinUuidNumber" placeholder="资产编号" clearable></el-input>
                    </el-form-item>
                     
                    <el-form-item :label="contents.inputTitle == 1 ? '资产名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.shangpinName" placeholder="资产名称" clearable></el-input>
                    </el-form-item>
                                         
                    <el-form-item :label="contents.inputTitle == 1 ? '维修状态' : ''">
                        <el-select v-model="searchForm.status" placeholder="请选择维修状态" clearable>
                            <el-option
                                label="未开始"
                                :value="0">
                            </el-option>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '资产类型' : ''">
                        <el-select v-model="searchForm.shangpinTypes" placeholder="请选择资产类型">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '存放地点' : ''">
                        <el-select v-model="searchForm.didianTypes" placeholder="请选择存放地点">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '开始资产库存' : ''">
                      <el-input v-model="searchForm.shangpinKucunNumberStartSearch"
                                placeholder="请输入开始资产库存" clearable></el-input>
                    </el-form-item>
                    <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                      =》
                    </el-form-item>
                    <el-form-item :label="contents.inputTitle == 1 ? '结束资产库存' : ''">
                        <el-input v-model="searchForm.shangpinKucunNumberEndSearch"
                                  placeholder="请输入结束资产库存" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('weixiu','新增') || $storage.get('role') === '用户'"
                                type="success"
                                icon="el-icon-plus"
                                @click="addOrUpdateHandler()"
                        >{{ $storage.get('role') === '用户' ? '申请设备维修' : '新增' }}</el-button>
                        &nbsp;
                        <el-button
                                v-if="isAuth('weixiu','删除')"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('weixiu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanzichan/upload/weixiuMuBan.xls"
                        >批量导入资产维修数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('weixiu','导入导出')"
                                ...
                                :on-success="weixiuUploadSuccess"
                                :on-error="weixiuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('weixiu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入资产维修数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('weixiu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "weixiu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-tag v-if="scope.row.status === 1" type="warning" size="mini">维修中</el-tag>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="审核状态">
                        <template slot-scope="scope">
                            <el-tag v-if="scope.row.shenheStatus === 0" type="warning" size="mini">待审核</el-tag>
                            <el-tag v-if="scope.row.shenheStatus === 1" type="success" size="mini">已批准</el-tag>
                            <el-tag v-if="scope.row.shenheStatus === 2" type="danger" size="mini">已拒绝</el-tag>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="220" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            <div class="operation-buttons">
                                <el-button v-if="isAuth('weixiu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                <el-button v-if="isAuth('weixiu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                                <el-button v-if="isAuth('weixiu','修改')" type="warning" icon="el-icon-s-tools" size="mini" @click="updateStatusHandler(scope.row)">状态</el-button>
                                <!-- 管理员审核按钮 -->
                                <template v-if="role === '管理员' && scope.row.shenheStatus === 0">
                                    <el-button type="success" icon="el-icon-check" size="mini" @click="handleApprove(scope.row)">批准</el-button>
                                    <el-button type="danger" icon="el-icon-close" size="mini" @click="handleReject(scope.row)">拒绝</el-button>
                                </template>
                            </div>
                        </template>
                    </el-table-column>
        ...
        <!-- 状态修改对话框 -->
        <el-dialog title="修改维修状态" :visible.sync="statusDialogVisible" width="30%">
            <el-form :model="statusForm" label-width="100px">
                <el-form-item label="资产名称">
                    <span>{{statusForm.shangpinName}}</span>
                </el-form-item>
                <el-form-item label="资产编号">
                    <span>{{statusForm.shangpinUuidNumber}}</span>
                </el-form-item>
                <el-form-item label="资产类型">
                    <span>{{statusForm.shangpinValue}}</span>
                </el-form-item>
                <el-form-item label="当前状态">
                    <el-tag v-if="statusForm.status === 0" type="info">未开始</el-tag>
                    <el-tag v-if="statusForm.status === 1" type="warning">维修中</el-tag>
                    <el-tag v-if="statusForm.status === 2" type="success">已完成</el-tag>
                </el-form-item>
                <el-form-item label="新状态">
                    <el-select v-model="statusForm.newStatus" placeholder="请选择维修状态">
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.weixiuNumberStartSearch != '' && this.searchForm.weixiuNumberStartSearch != undefined) {
                    params['weixiuNumberStartSearch'] = this.searchForm.weixiuNumberStartSearch
                }
                if (this.searchForm.weixiuNumberEndSearch != '' && this.searchForm.weixiuNumberEndSearch != undefined) {
                    params['weixiuNumberEndSearch'] = this.searchForm.weixiuNumberEndSearch
                }
                        
                params['weixiuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            weixiuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "weixiu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            updateStatusHandler(row) {
                this.statusForm = {
                    ...
                    shangpinValue: row.shangpinValue
                };
            ...
            confirmUpdateStatus() {
                if (this.statusForm.newStatus === null) {
                    this.$message.warning("请选择新状态");
                    return;
                }
                if (this.statusForm.status === this.statusForm.newStatus) {
                    this.$message.warning("新状态与当前状态相同，无需更新");
                    return;
                }
                this.statusDialogVisible = false;
                this.$http({
                    url: `weixiu/updateStatus`,
                    method: "post",
                    data: {
